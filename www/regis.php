...
if($_POST['reg'])
{
	...
	$t=mysqli_num_rows($r);
		...
		{
		$ret = mysqli_query($con, "INSERT INTO userinfo(user_name,password,mobile,email,gender,hobbies,dob,image) values('{$_POST['un']}','{$_POST['pwd']}','{$_POST['mob']}','{$_POST['eid']}','{$_POST['gen']}','$hobbies','$dob',
		'$imgpath')");
		if(!$ret){die("Error adding user".mysqli_error($con));}
		//mkdir("userImages/$un");
